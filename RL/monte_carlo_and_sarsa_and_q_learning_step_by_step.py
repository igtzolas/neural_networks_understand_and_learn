...
gen_mc = mc_control_epsilon_greedy(env)
gen_sarsa = sarsa(env)
gen_q = q_learning(env)

def execute_1_episode_step(episode_id_step):
    global line_options
    visualize(Q_values_heatmap_monte_carlo, Q_values_monte_carlo, monte_carlo_window_name, episode_id_step)
    visualize(Q_values_heatmap_sarsa, Q_values_sarsa, sarsa_window_name, episode_id_step)
    visualize(Q_values_heatmap_q_learning, Q_values_q_learning, q_learning_window_name, episode_id_step)


episode_id = 1
def step():
    sarsa_complete = False
    mc_complete = False
    q_learning_complete = False
    global episode_id
    while not (sarsa_complete and q_learning_complete and q_learning_complete):
        if not mc_complete:
            try:
                next(gen_mc)
            except StopIteration:
                mc_complete = True
                print("MC Reached 1500 steps")
        if not sarsa_complete:
            try:
                next(gen_sarsa)
            except StopIteration:
                sarsa_complete = True
                print("Sarsa complete in {}".format(episode_id))
        if not q_learning_complete:
            try:
                next(gen_q)
            except StopIteration:
                q_learning_complete = True
                print("Q Learning complete in {}".format(episode_id))
        execute_1_episode_step(episode_id)
        episode_id += 1
        yield
