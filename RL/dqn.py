...
def train(nn_son, nn_parent, training_loops_per_episode = 10):
    for _ in range(training_loops_per_episode):    
        ...
        q_values_parent_advice = rewards.reshape(-1, 1) + gamma * nn_parent.forward(next_states).max(dim=1, keepdim=True)[0] * is_done_mask.reshape(-1, 1)  
...
for generation_id in range(n_generations):
    ...
    for episode_id in range(n_episodes_per_generation):
        ...
        line_options["title"] = "DQN - Episode Score"
        ...
        vis.line(X=np.array(running_episode).reshape(-1, 1), Y=np.array(episode_score).reshape(-1, 1) , env = "main", win = "DQN - Episode Score", opts= line_options, update="append")
    ...
    line_options["title"] = "DQN - Average Generation Score"
    line_options["xlabel"] = "# Generations"
    line_options["ylabel"] = "Average Generation Score"
    vis.line(X=np.array(generation_id).reshape(-1, 1), Y=np.array(average_generation_score).reshape(-1, 1) , env = "main", win = "DQN - Average Generation Score", opts= line_options, update="append")
